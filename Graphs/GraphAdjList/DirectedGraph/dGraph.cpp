...
template<class VertexType>
std::vector< std::pair<VertexType, double> > dGraph<VertexType>::getAdjVertices(const VertexType & v1) const{
...
bool dGraph<VertexType>::depthFirst(const VertexType & root_data, GraphTraveler<VertexType> * traveler) {
    ...
    if(traveler != nullptr)
        traveler->starting_vertex(root_vertex->getVertex()->getData());
...
template<class VertexType>
typename dGraph<VertexType>::dist_prev_pair dGraph<VertexType>::dijkstras(const VertexType & source) {
...
template<class VertexType>
std::pair<std::vector<VertexType>, double> dGraph<VertexType>::dijkstrasComputePath(const VertexType & src, const VertexType & dest) {
