...
*               uEdge is a simple data structure that contains references to the endpoint that it points to, it doesn't
*               need to know where it is pointing from because it will be in a list associated with the other vertex.
*               This edge will also contain a weight. This wieght will have a default value, so if the user wishes
*               to implement an unweighted graph they can still use this class.
**/
...
public:
    ...
    Edge(Vertex<VertexType> *, Vertex<VertexType> *,
